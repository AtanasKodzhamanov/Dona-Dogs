...
from DogShelter.web.forms import (
    SubscribeForm,
    DogFilterForm,
    vaStatusForm,
    genderFilterForm,
...
class BaseView(TemplateView):
    ...
    def get_common_context_data(self):
        ...
        context["content_containter_pics"] = [f"about_pic_{i}" for i in range(1, 11)]
...
class HomeView(BaseView):
    ...
    def get_common_context_data(self):
        ...
        data_dogs = Dog.objects.filter(status__in=["Active", "Sick", "New"]).order_by(
            "?"
        )
    ...
    def post(self, request):
        return handle_form_post(
            request, template_name="home.html", context=self.get_common_context_data()
        )
...
class DonationStoryView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        return handle_form_post(
            ...
            context={"date_pk": self.kwargs["date_pk"], **self.get_context_data()},
        )
...
class GalleryView(BaseView):
    ...
    def get_common_context_data(self):
        ...
        context["new_dogs"] = Dog.objects.filter(status__in=["New"])
        context["active_dogs"] = Dog.objects.filter(
            status__in=["Active", "Sick"]
        ).order_by("-arrival_year")
        context["data_people"] = set(
            Dog.objects.values_list("va_name_bg", "va_name_eng").exclude(va_name_eng="")
        )
        context["subscribe_form"] = SubscribeForm(self.request.POST or None)
...
class ClinicView(BaseView):
    ...
    def post(self, request):
        return handle_form_post(
            request, template_name="clinic.html", context=self.get_common_context_data()
        )
...
class AdoptionsView(BaseView):
    ...
    def get_common_context_data(self):
        ...
        years = sorted(set(dogs.values_list("adoption_year", flat=True)), reverse=True)
        field_names = [
            "pic_2",
            "pic_3",
            "pic_4",
...
class DonationsView(BaseView):
    ...
    def get_common_context_data(self):
        ...
        last_month = today.replace(day=1) - datetime.timedelta(days=1)
        last_month_cl = (
            "(" + last_month.strftime("%m") + " - " + last_month.strftime("%Y") + ")"
        )
        data_donations = set(
            Donation.objects.values_list("person_name_bg", "person_name_eng")
            .filter(date=last_month)
            .order_by("person_name_bg")
        )
